...
void Sem_init(sem_t *sem, int pshared, unsigned int value)
{
    ...
    if ((ds = dispatch_semaphore_create(value)) == NULL)
...
/*
 * rio_read - This is a wrapper for the Unix read() function that transfers
 * min(n, rio_cnt) bytes from an internal buffer to a user buffer, where n is
 * the number of bytes requested by the user and rio_cnt is the number of
 * unread bytes in the internal buffer. On entry, rio_read() refills the
...
open_clientfd(char *hostname, char *port)
{
	...
	hints.ai_flags = AI_NUMERICSERV;	/* ... using a numeric port
						 * arg. */
	hints.ai_flags |= AI_ADDRCONFIG;	/* Recommended for
						 * connections */
	...
	for (p = listp; p; p = p->ai_next) {
		...
		if (connect(clientfd, p->ai_addr, p->ai_addrlen) != -1)
			break;	/* Success */
		if (close(clientfd) < 0) {	/* Connect failed, try
						 * another */
